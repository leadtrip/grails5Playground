...
@Integration
class PlayerControllerFunctionalSpec extends Specification {
    ...
    void 'test save validation'() {
        when:
            ...
            client.toBlocking().exchange(HttpRequest.POST('/player/save', json)
                    .accept(MediaType.APPLICATION_JSON_TYPE), Map)
        ...
        then:
            ...
            e.response.body().errors.find { it.field == 'game' }.message ==
                    'Property [game] of class [class wood.mike.Player] cannot be null'
    }
}
